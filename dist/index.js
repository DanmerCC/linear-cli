...
async function ensureAuthenticated() {
    let token = (0, config_1.getLinearToken)();
    if (!token) {
        ...
        const answers = await inquirer_1.default.prompt([
            {
                ...
                validate: (input) => input.length > 0 || "El token no puede estar vacío.",
            },
        ]);
        (0, config_1.setLinearToken)(answers.token);
...
program
    ...
    try {
        const client = (0, linear_client_1.getLinearClient)();
        const projects = await client.projects();
        const project = projects.nodes.find((p) => p.name.toLowerCase().includes(projectName.toLowerCase()) || p.id === projectName);
        if (!project) {
